...
Two shops MAY hold their databases on different hosts, and MAY hold them on the
same host. One shop's identity SHALL NOT depend on either. Everything this
requirement demands SHALL hold whether the two databases sit on one provider or
on two, and a shop moving from one provider to another SHALL NOT change what any
other shop reads or writes.
...
#### Scenario: Two shops on the same database host

- **WHEN** both shops' databases sit on one provider
- **THEN** each still reads and writes only its own database
- **AND** neither shop's orders or subscribers appear on the other

#### Scenario: One shop changes provider

- **WHEN** one shop's database moves to a different provider
- **THEN** the other shop is unaffected and continues to read and write its own database
