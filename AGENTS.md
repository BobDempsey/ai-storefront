...
`server/types/database.ts` is generated by `npm run db:types` from the live
project and committed. `useSupabase()` returns `SupabaseClient<Database>`, so a
renamed or dropped column is a type error in every query that reads it.

Regenerate after any change to `supabase/schema.sql` that you have applied, and
put the regenerated file in the same commit as the SQL. The script reads
...
```
npm test            typecheck + tests/unit, nothing external, about 20 seconds
npm run typecheck   types only, through vue-tsc
npm run lint        ESLint with type-aware rules, about 17 seconds
npm run check       typecheck + lint + tests/unit, the full local pass
...
`npm run lint` is not part of `npm test`: the two together ran 36 seconds,
past the budget, so the lint moved to its own script and to `npm run check`.
Run `check` before you call a piece of work finished. The config is correctness
rules only, no formatting, and a rule turned off carries a comment saying why.
